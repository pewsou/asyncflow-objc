...
#import "ASFKComposePar.h"
@interface ASFKComposePar()
@property dispatch_queue_t exQ;
@property NSLock* lock;
@end
@implementation ASFKComposePar
-(id)init{
    self = [super init];
    if(self){
        NSString* rand=[[self generateRandomNumber]stringValue];
        self.exQ = dispatch_queue_create([[@"com.compose.par.1." stringByAppendingString:rand]cStringUsingEncoding:NSASCIIStringEncoding], DISPATCH_QUEUE_CONCURRENT);
        rand=nil;
